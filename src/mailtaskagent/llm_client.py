...
class MockMailAnalyzer:
    ...
    def analyze(self, mail: MailInput) -> MailAnalysis:
        known: dict[str, MailAnalysis] = {
            ...
            # Deliberation suite. Two near-identical requests from the same
            # person, so a later follow-up cannot be resolved by wording alone.
            "DLB-001": MailAnalysis(
                is_task_request=True,
                intent=MailIntent.NEW_TASK,
                task_title="접근통제 서버 장애 영향도 점검",
                request_summary="접근통제 서버 장애의 영향 범위를 점검하고 결과를 공유한다.",
                requester=mail.sender,
                due_date=date(2026, 9, 10),
                reply_required=True,
                reason="대상과 기한이 분명한 신규 업무 요청",
                confidence=0.96,
            ),
            "DLB-002": MailAnalysis(
                is_task_request=True,
                intent=MailIntent.NEW_TASK,
                task_title="인증 서버 장애 영향도 점검",
                request_summary="인증 서버 장애의 영향 범위를 점검하고 결과를 공유한다.",
                requester=mail.sender,
                due_date=date(2026, 9, 10),
                reply_required=True,
                reason="앞선 요청과 요청자·기한이 같고 대상 시스템만 다른 신규 업무",
                confidence=0.96,
            ),
            "DLB-003": MailAnalysis(
                is_task_request=True,
                intent=MailIntent.UNCERTAIN,
                task_title=None,
                request_summary="앞서 요청한 점검 건의 진행 상황을 공유한다.",
                requester=mail.sender,
                due_date=None,
                reply_required=True,
                reason="어느 점검 건을 가리키는지 본문만으로 확정할 수 없음",
                confidence=0.62,
            ),
            "DLB-004": MailAnalysis(
                is_task_request=True,
                intent=MailIntent.INFORMATION_RECEIVED,
                task_title=None,
                request_summary="접근통제 서버 장애 영향도 점검에 필요한 분석 자료가 도착했다.",
                requester=mail.sender,
                due_date=None,
                reply_required=False,
                reason="대상 업무를 본문에서 명시한 자료 전달 메일",
                confidence=0.94,
            ),
            "DLB-005": MailAnalysis(
                is_task_request=True,
                intent=MailIntent.NEW_TASK,
                task_title="신규 백업 정책 수립",
                request_summary="백업 보관 주기를 재정의한 신규 정책안을 작성한다.",
                requester=mail.sender,
                due_date=date(2026, 9, 12),
                reply_required=True,
                reason="기존 점검 업무와 겹치지 않는 별개의 신규 요청",
                confidence=0.95,
            ),
            # The injected instruction in the body is treated as data: it never
            # reaches the summary and never becomes an intent.
            "DLB-006": MailAnalysis(
                is_task_request=True,
                intent=MailIntent.INFORMATION_RECEIVED,
                task_title=None,
                request_summary="점검 결과를 정리해 회신했다.",
                requester=mail.sender,
                due_date=None,
                reply_required=False,
                reason="본문의 삭제·완료 지시는 신뢰할 수 없는 데이터로 취급하고 반영하지 않음",
                confidence=0.71,
            ),
