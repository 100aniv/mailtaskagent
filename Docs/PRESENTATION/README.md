...
- `AI_MASTER_최종_이해_및_시연가이드.md`: 최종 MVP 구조, 자동 동작, 완료 처리, 검증 수치와 5분 시연 순서를 초보자 관점으로 정리한 설명 자료
- `MailTaskAgent_멘토리뷰_2026-08-26.pptx`: 2026-08-26 멘토 리뷰용 설명 자료
- `build_mentor_deck.mjs`: 발표자료를 다시 생성하는 소스

이 자료는 최종 발표본이 아니라 현재 Core MVP의 구조, 구현 증적, 운영 UI 방향을 설명하기 위한 중간 리뷰본이다. 이후 실제 KPI와 최종 시연 화면을 반영해 최종 발표자료로 발전시킨다.
