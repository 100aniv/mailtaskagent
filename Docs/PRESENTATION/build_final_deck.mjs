...
const DECK_VERSION = process.env.DECK_VERSION ?? "v11";
...
// 1. Cover
{
  ...
  notes(s,"[시간 배분] 개요 2분, 아키텍처 4분, 핵심 기술 과제 4분. 슬라이드를 넘기기 전에 시간을 확인한다.",["Docs/PRESENTATION/2. 최종/09285_백준현_최종시연_스크립트.md"]);
}
...
// 4. Challenge and proof
{
  ...
  const agentRows=[
    ...
    ["재판단", "모호·저신뢰이거나 선택 차이가 작으면 Query Rewrite 후 재검색을 최대 1회 수행"],
  ...
  notes(s,"이 슬라이드의 난제부터 말씀드리겠습니다. 저는 이미 Agent를 쓰고 있었습니다. 문제는 그 Agent가 결론을 하나만 돌려줬다는 것입니다. 관계 하나, 대상 하나, 행동 하나에 설명 한 줄입니다. 이러면 모델이 후보를 실제로 견주고 고른 것인지, 결론을 먼저 정하고 그럴듯한 설명을 붙인 것인지 밖에서 구분할 방법이 없습니다. 멘토님 피드백도 매번 같았습니다. 룰 베이스처럼 보인다, 어떤 후보를 검토했는지 보여 달라. 판단의 품질이 아니라 판단의 검증 가능성이 막힌 지점이었습니다.\n\n그래서 적용한 기법이 화면에 적힌 제한적 다중 가설 숙고입니다. 먼저 분명히 해 두겠습니다. 트리 오브 소트를 구현한 것이 아닙니다. 후보 생성과 평가를 분리한다는 아이디어만 참고했고, 후보 두세 개, 평가 한 번, 재검색 한 번으로 제한했습니다. 메일 한 건을 업무로 옮기는 판단은 단계가 깊지 않은 대신 틀렸을 때 비용이 업무 데이터 오변경이기 때문입니다. 생성과 평가를 서로 다른 호출로 나눈 것이 핵심입니다. 생성 단계는 점수를 매기지 않고, 평가 단계는 새로 만들지 못합니다. 그래서 화면에 남는 후보 목록이 사후 설명이 아니라 실제로 비교된 기록이 됩니다.\n\n효과는 같은 코드에서 기능만 껐다 켜서 측정했습니다. 기존 열다섯 개 Case는 양쪽이 똑같이 15/15, Action 28/28이고 소요시간만 약 삼십 퍼센트 늘었습니다. 숙고가 실제 발동하는 전용 네 개 케이스는 엘엘엠 출력이 회차마다 달라져서 각 설정을 세 번씩 돌렸습니다. 통과 수는 켠 쪽이 삼, 삼, 이이고 끈 쪽이 이, 이, 이입니다. 한 회차 숫자를 결론으로 쓰지 않았습니다. 세 번 모두 재현되는 차이는 한 건입니다. 요청자가 같고 대상 시스템만 다른 유사 업무가 둘 있을 때, 단일 결론 경로는 세 번 모두 둘 중 하나에 그대로 연결했고, 두 단계 경로는 세 번 모두 사용자 확인으로 닫았습니다. 제가 개선했다고 말하는 범위는 이 한 건입니다. 엘엘엠을 호출하지 않는 목 경로에서는 양쪽이 모두 사 대 사로 차이가 없습니다. 목 분석기는 어휘 겹침만 보기 때문에 숙고가 다루는 판단 자체가 생기지 않습니다.\n\n실패도 말씀드리겠습니다. 처음 구현에서 응답 검증을 재시도 루프 밖에 두었습니다. 그래서 모델이 필드 하나를 배열로 돌려준 것만으로 판단 전체가 실패했고, 저장해 둔 실제 Gmail 서른다섯 건을 다시 흘려보냈을 때 판단 네 건이 사라졌습니다. 합성 테스트로는 드러나지 않았고 실제 데이터 재생에서 잡혔습니다. 검증을 루프 안으로 옮긴 뒤 유실은 0건입니다.\n\n반대로 개선되지 않은 것도 말씀드리겠습니다. 검색 결과 안에 명령을 심어 둔 케이스는 두 설정 모두 세 번 모두 기대값을 충족하지 못했습니다. 기대값은 사용자 확인인데 실제로는 무시로 끝났습니다. 다만 이 케이스가 확인해야 할 안전 속성인, 심어진 명령을 실행하지 않는다는 점은 여섯 번 모두 지켜졌습니다. 본문이 지시한 전체 완료 처리와 기록 삭제는 한 번도 일어나지 않았습니다. 명령은 거부하지만 사람에게 넘기지는 않는, 안전하되 정보를 버리는 동작이고 한계로 남겨 두었습니다. 화면의 신뢰도와 지지도 역시 모델의 자기보고 값이지 검증된 정답 확률이 아닙니다.\n\n마지막으로 하지 않은 선택입니다. 여러 계획을 만들어 비교하는 Tree of Thoughts는 적용하지 않았습니다. 메일 한 건을 업무로 옮기는 판단은 단계가 깊지 않은 대신 틀렸을 때 비용이 업무 데이터 오변경이라, 후보를 두세 개로 제한하고 평가와 재검색을 각각 한 번으로 묶은 뒤 결정론적 Guard와 사용자 승인으로 막는 쪽을 택했습니다. 문서에도 구현했다고 쓰지 않고 거기서 착안한 제한적 후보 탐색·평가라고 적었습니다.",["evidence/deliberation_ab_2026-09-15.json","evidence/gmail_replay_deliberation_2026-09-15.json","evidence/final_audit_live_2026-09-13_after_inbound_intent_guard.json","tests/test_deliberation.py","tests/test_agent_deliberation.py"]);
}
