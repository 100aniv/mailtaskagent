...
LLM을 호출하지 않는 결정론적 Mock에서도 같은 차이가 같은 Case, 같은 실패 방식으로 나온다(ON 4/4 · OFF 3/4). 회차 변동이 없는 경로에서 재현되므로 이 차이는 모델 출력 운이 아니라
판단 구조의 차이라고 말할 수 있다.

전용 Case 4건의 전체 통과 수는 ON이 3·4·3, OFF가 3·2·3으로 회차마다 다르다. 검색 Context
Prompt Injection Case(`DLB-04`)가 두 설정 모두에서 흔들리기 때문이며, 이를 숙고의 효과로도
부작용으로도 말하지 않는다. 기존 15 Case는 양쪽 다 15/15로 같고 소요시간만 늘었다. 성능이
올라갔다는 주장이 아니라, 잘못된 업무에 조용히 붙는 경우가 사람에게 넘어갔다는 뜻이다.
