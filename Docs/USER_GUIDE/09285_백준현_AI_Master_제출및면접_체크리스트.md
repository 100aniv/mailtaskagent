...
정의된 전용 Case 4건에서 숙고 ON이 4/4, OFF가 3/4였다. 차이는 한 건이다. 요청자가 같고 대상
시스템만 다른 유사 업무 2건이 있을 때, 단일 결론 경로는 둘 중 하나에 그대로 연결했고 2단계
경로는 지지도 0.55 대 0.45로 차이가 기준에 미달해 재검색 후에도 갈리지 않자 사용자 확인으로
닫았다. 기존 15 Case는 양쪽 다 15/15로 같고 소요시간만 약 30퍼센트 늘었다. 성능이 올라갔다는
주장이 아니라, 잘못된 업무에 조용히 붙는 경우가 사람에게 넘어갔다는 뜻이다.
