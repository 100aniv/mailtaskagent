...
- 실제 업무 모드 Navigation을 `홈`, `업무`, `검토함`, `설정` 4개로 단순화했다.
  Mail 처리와 활동 기록은 설정의 목적별 하위 화면으로 이동했고 기존 시연 모드는 별도로 유지한다.
...
|---|---|---|
| 실제 업무 UI·Task 직접 관리 | 구현·자동 테스트 완료 | 홈/업무/검토함/설정, 직접 생성·수정·완료 |
