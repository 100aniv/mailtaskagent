...
* **Bounded Multi-Hypothesis Deliberation A/B:** 같은 코드에서 `AGENT_DELIBERATION_ENABLED`만
  ...
  모두 15/15 실행 단위·28/28 Action 단계·Intent 13/15로 동일했고 소요시간만 101.0초에서 131.9초로
  늘었습니다. 숙고가 실제 발동하는 전용 4 Case에서는 숙고 ON이 4/4, OFF가 3/4였습니다. 차이는
  요청자가 같고 대상 시스템만 다른 유사 업무 2건이 있는 상황의 후속 Mail 한 건으로, 단일 결론
  경로는 둘 중 하나에 그대로 연결했고 2단계 경로는 지지도 0.55 대 0.45로 차이 0.10이 기준 0.15에
  미달해 재검색 후에도 갈리지 않자 사용자 확인으로 닫았습니다. 잘못된 업무에 조용히 붙는 대신
  사람에게 넘어간다는 점이 이 기능의 목적입니다.
