...
자동 테스트는 Mail Schema, LLM Schema Retry, Ground Truth KPI 계산, 중복 방지, Transaction Rollback, Secret 마스킹, 7개 Action, 5개 상태, 사용자 승인·수정·거절, Task 직접 수정을 포함하여 `44 passed`를 기록했습니다.
...
* **남은 고도화:** 동일 Case의 수동 대비 처리시간을 실제 측정합니다. 현재 Task 연결은 8/8로 목표를 달성했으므로 제한적 LLM 의미 Matching은 추가하지 않고, 향후 정확도 미달이 확인될 때만 검토합니다.
