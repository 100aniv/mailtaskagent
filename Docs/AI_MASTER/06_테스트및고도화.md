...
자동 테스트는 Mail Schema, LLM Schema Retry, Ground Truth KPI 계산, 동일 Case 시간 비교, Thread·Task Context, 허용 상태 전이, 중복 방지, Transaction Rollback, Secret 마스킹, 7개 Action, 5개 상태, 사용자 승인·수정·거절, Task 직접 수정과 제품형 Dashboard 전체 Mail 처리를 포함하여 `50 passed`를 기록했습니다.
