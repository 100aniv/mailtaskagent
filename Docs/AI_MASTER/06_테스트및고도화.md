...
자동 테스트는 Mail Schema, LLM Schema Retry, Ground Truth KPI 계산, 동일 Case 시간 비교, Thread·Task Context, 허용 상태 전이, 중복 방지, Transaction Rollback, Secret 마스킹, 7개 Action, 5개 상태, 사용자 승인·수정·거절, Task 직접 수정, 제품형 Dashboard 전체 Mail 처리와 읽기 전용 Gmail Adapter Contract·빈 상태 UI·History 및 로그 요약을 포함하여 `58 passed`를 기록했습니다.
...
* **남은 고도화:** Streamlit에 구현한 SC-001·002·003 수동 타이머로 실제 사용자가 6개 Action을 모두 맞힌 실행의 수동 대비 처리시간을 측정합니다. 현재 Task 연결은 8/8로 목표를 달성했으므로 제한적 LLM 의미 Matching은 추가하지 않고, 향후 정확도 미달이 확인될 때만 검토합니다.
