...
* **선택적 Gmail Adapter Contract:** 합성 Gmail API Payload와 Fake Service로 INBOUND/OUTBOUND, Thread ID, 한글 주소, Text/HTML 본문, 오래된 Mail 우선 정렬, 제한 Query와 최대 건수 Guard를 검증했습니다. 실제 OAuth 연결과 제한 Label Query 0건 응답도 확인했지만, 합성 Mail 처리 E2E 완료로 확대 기록하지 않습니다.
