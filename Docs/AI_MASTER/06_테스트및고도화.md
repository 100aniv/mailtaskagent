...
자동 테스트는 Mail Schema, LLM Schema Retry, Ground Truth KPI 계산, 동일 Case 시간 비교, Thread·Task Context, 허용 상태 전이, 중복 방지, Transaction Rollback, Secret 마스킹, 7개 Action, 5개 상태, 사용자 승인·수정·거절, Task 직접 수정, 제품형 Dashboard 전체 Mail 처리, 실제 업무/시연 모드 및 DB 격리와 읽기 전용 Gmail Adapter Contract·빈 상태 UI·History 및 로그 요약을 포함하여 `60 passed`를 기록했습니다.
...
* **운영/시연 모드 격리:** 첫 화면의 두 모드 진입, 실제 업무 모드의 4개 핵심 Tab, 시연 모드의 품질 검증·데모 도구, 모드별 SQLite 경로 분리를 자동 검증했습니다.

* **선택적 Gmail Adapter 및 Live E2E:** 합성 Gmail API Payload와 Fake Service로 INBOUND/OUTBOUND, Thread ID, 한글 주소, Text/HTML 본문, 오래된 Mail 우선 정렬, 제한 Query와 최대 건수 Guard를 검증했습니다. 실제 OAuth 연결 후 제한 Label의 비식별 합성 Mail 2건을 회사 LLM Live로 처리해 `CREATE_TASK` 1건, 동일 Thread `UPDATE_TASK` 1건, 기한 변경과 실패 0건을 확인했습니다.
