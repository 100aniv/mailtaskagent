...
자동 테스트는 Mail Schema, LLM Schema Retry, Ground Truth KPI 계산, 동일 Case 시간 비교, Thread·Task Context, 허용 상태 전이, 중복 방지, Transaction Rollback, Secret 마스킹, 7개 Action, 5개 상태, 사용자 승인·수정·거절, Task 직접 수정, 제품형 Dashboard 전체 Mail 처리와 읽기 전용 Gmail Adapter Contract를 포함하여 `55 passed`를 기록했습니다.
...
* **선택적 Gmail Adapter Contract:** 실제 계정 대신 합성 Gmail API Payload와 Fake Service로 INBOUND/OUTBOUND, Thread ID, 한글 주소, Text/HTML 본문, 오래된 Mail 우선 정렬, 제한 Query와 최대 건수 Guard를 검증했습니다. 실제 OAuth 연결 완료로 확대 기록하지 않습니다.
