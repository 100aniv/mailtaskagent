...
자동 테스트는 Mail Schema, LLM Schema Retry, Ground Truth KPI 계산, 동일 Case 시간 비교, Thread·Task Context, 허용 상태 전이, 중복 방지, Transaction Rollback, Secret 마스킹, 7개 Action, 5개 상태, 사용자 승인·수정·거절, Task 직접 수정, 제품형 Dashboard, 실제 업무/시연 모드와 DB 격리, Gmail 수신·발신 Thread 추적, 운영 자동화·복구, SQLite WAL·동시 동기화 단일 실행 잠금 및 업무별 History UI를 포함하여 `122 passed`를 기록했습니다.

위 `122 passed`, Live 15/15와 28/28은 Task Context RAG 적용 전 시연 가능한 Core 기준선의
증적입니다. 최종 MVP에서는 다른 Thread·다른 표현의 동일 Task 연결, 저신뢰 후 Query Rewrite
성공, 재검색 후 `ASK_USER`, 후보 밖 Task ID 차단, RAG API·Schema 실패 시 DB 무변경,
완료·취소·기한 단축 승인 Gate와 기존 전체 회귀를 추가합니다. 신규 Evidence와 전체 pytest가
통과하기 전에는 RAG 또는 최종 MVP를 완료로 기록하지 않습니다.
...
* **남은 고도화:** 멘토 피드백의 경량 Task Context Agentic RAG와 최대 1회 Query Rewrite,
  Fail-closed 테스트를 최종 MVP 잔여 범위로 구현합니다. 시간 절감 효과는 Microsoft와
