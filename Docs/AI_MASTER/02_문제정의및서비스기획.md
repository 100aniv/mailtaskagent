...
- Python M-02: Thread Metadata와 제목·요청자·요청요약 기반 Task 후보 검색
- Python M-03: 현재 Mail Context와 Task 상태에 따른 최종 Action 결정
...
- Metadata와 Token 기반 Matching 정확도가 부족한 경우에만 제한적인 의미 기반 Matching을 후속 고도화로 검토한다.
...
- 첫 화면에서 `실제 업무 모드`와 `MVP 시연 모드`를 선택한다. 동일 Agent Core를 사용하되 실제 업무 DB와 시연 DB를 분리하고, 실제 업무 화면에서는 업무 현황·메일 처리함·확인 필요·운영 로그만 제공한다.
- Streamlit Dashboard의 `오늘의 업무`에서 처리 현황, 우선 처리 업무, 기한·회신 대기 주의 항목과 사용자 확인 대기를 먼저 확인하고, Mail 처리함, 운영 로그와 시연 모드의 품질 검증 화면에서 세부 결과를 추적한다.
- 사용자는 Task를 조회·수정하고 중요 변경을 승인·수정·거절할 수 있다.
