...
Python과 회사 LLM API를 이용하여 M-01 Mail 의미·Intent 구조화, M-02 Task 후보 검색, M-03 최종 Action 결정, Application Logic의 Validation·DB 반영, M-04 Task·History 관리와 M-05 Streamlit Dashboard를 연결한 E2E를 구현한다. 회사 LLM Live 15/15 실행 단위, 28/28 Action 단계, 업무 요청 분류 15/15, 요청사항·기한 추출 26/26, 기존 Task 연결 8/8과 pytest 44 passed를 확보했다. 수동 처리시간 Baseline은 측정 완료 전까지 달성값으로 기록하지 않는다.
