...
구현했다. 현재 시연 기준선은 Metadata·Token 기반 Task Matching이며, 멘토 피드백을 반영한
경량 Task Context Agentic RAG는 최종 MVP 잔여 과제로 설계했다. 이 기능은 아직 코드·테스트
완료 전이며, 사내 문서검색 RAG·Vector DB·Multi-Agent와 구분한다.
...
- SQLite Task·Mail·History 기반 경량 Retrieval, Task Context Agent와 최대 1회 Query Rewrite
...
구현·검증했다. 별도 Framework 없이 SQLite Context를 제한적으로 검색하는 Task Context RAG는
최종 MVP 잔여 범위다. LangGraph는 현재 복잡도에서 도입하지 않으며, Microsoft Graph/Outlook,
...
Python과 회사 LLM API를 이용하여 M-01 Mail 의미·Intent 구조화, M-02 Task 후보 검색, M-03 최종 Action 결정, Application Logic의 Validation·DB 반영, M-04 Task·History 관리와 M-05 Streamlit Dashboard를 연결한 E2E를 구현한다. 회사 LLM Live 15/15 실행 단위, 28/28 Action 단계, 업무 요청 분류 15/15, 요청사항·기한 추출 26/26, 기존 Task 연결 8/8을 확보했다. Core 기준선과 Gmail 개인 파일럿, 제품형 Dashboard, History·로그, 운영/시연 격리, SQLite WAL·동시 동기화 단일 실행 잠금과 안전 실패 회귀를 포함한 전체 자동 테스트는 pytest 122 passed다. 읽기 전용 테스트 Gmail Adapter는 Core와 분리해 구현했고 실제 OAuth 연결 후 별도 테스트 계정의 비식별 합성 Mail 20건을 회사 LLM Live로 처리해 20/20 수용시험과 수신·발신 Thread Lifecycle, 재조회 중복 20건·실패 0건을 확인했다. 실제 업무 모드는 홈·내 업무·검토 요청·자동화 설정·운영 상태·설정으로 분리하고, 업무별 Mail 타임라인과 변경 이력을 제공한다. 동일 Case의 수동 처리시간 측정 UI를 구현했으며 실제 Baseline은 측정 완료 전까지 달성값으로 기록하지 않는다. 최종 MVP 완료 전에는 다른 표현의 동일 Task를 제한 Context로 판단하는 Task Context Agent, 최대 1회 Query Rewrite와 Fail-closed 테스트를 추가한다.

- 비즈니스 목표:

메일로 유입되는 업무 요청을 Task로 구조화하고 요청사항·기한·회신 대기·완료 등 Lifecycle을 지속 관리하여 업무 누락과 반복적인 Mail 확인 부담을 줄이는 개인 업무관리 MVP를 완성한다. 현재 시연 가능한 Core 기준선의 15개 합성 Mail Ground Truth에서는 업무 요청 분류와 신규/기존 Task 연결 목표를 달성했다. 멘토 피드백의 Task Context RAG를 구현·검증한 뒤 최종 MVP 완료로 판정하며, 업무 정리시간 30% 이상 단축은 실제 수동 처리 Baseline 확보 후 확정한다.
