...
Python과 회사 LLM API를 이용하여 M-01 Mail 의미·Intent 구조화, M-02 Task 후보 검색, M-03 최종 Action 결정, Application Logic의 Validation·DB 반영, M-04 Task·History 관리와 M-05 Streamlit Dashboard를 연결한 E2E를 구현한다. 회사 LLM Live 15/15 실행 단위, 28/28 Action 단계, 업무 요청 분류 15/15, 요청사항·기한 추출 26/26, 기존 Task 연결 8/8과 제품형 Dashboard·Gmail Adapter Contract·빈 상태 UI·History 및 로그 요약·운영/시연 모드 격리 회귀를 포함한 pytest 60 passed를 확보했다. 첫 화면에서 실제 업무 모드와 MVP 시연 모드를 선택하고 동일 Agent Core를 공유하되 실제 업무 DB와 시연 DB는 분리한다. 읽기 전용 테스트 Gmail Adapter는 Core와 분리해 구현했고 실제 OAuth 연결 후 제한 Label의 비식별 합성 Mail 2건을 회사 LLM Live로 처리해 `CREATE_TASK`와 동일 Thread `UPDATE_TASK`, 기한 변경과 실패 0건을 확인했다. 동일 Case의 수동 처리시간 측정 UI를 구현했으며 실제 Baseline은 측정 완료 전까지 달성값으로 기록하지 않는다.
