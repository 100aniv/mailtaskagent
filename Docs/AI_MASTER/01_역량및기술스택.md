...
선택적 고도화였던 테스트 Gmail Input Adapter와 UI 사용성 고도화는 Core 완료 후 구현·검증했다. 제한적인 Task Matching 고도화와 필요성이 확인될 경우의 LangGraph는 현재 정확도와 복잡도 기준으로 도입하지 않았다. Microsoft Graph/Outlook, n8n, 사내 서버/VM 및 운영 알림은 AI Master Core 완료 이후 Post-MVP 사내 적용 단계에서 검토한다.
...
Python과 회사 LLM API를 이용하여 M-01 Mail 의미·Intent 구조화, M-02 Task 후보 검색, M-03 최종 Action 결정, Application Logic의 Validation·DB 반영, M-04 Task·History 관리와 M-05 Streamlit Dashboard를 연결한 E2E를 구현한다. 회사 LLM Live 15/15 실행 단위, 28/28 Action 단계, 업무 요청 분류 15/15, 요청사항·기한 추출 26/26, 기존 Task 연결 8/8을 확보했다. Core와 Gmail 개인 파일럿, 제품형 Dashboard, History·로그, 운영/시연 격리, SQLite WAL·동시 동기화 단일 실행 잠금과 안전 실패 회귀를 포함한 전체 자동 테스트는 pytest 122 passed다. 읽기 전용 테스트 Gmail Adapter는 Core와 분리해 구현했고 실제 OAuth 연결 후 별도 테스트 계정의 비식별 합성 Mail 20건을 회사 LLM Live로 처리해 20/20 수용시험과 수신·발신 Thread Lifecycle, 재조회 중복 20건·실패 0건을 확인했다. 실제 업무 모드는 홈·내 업무·검토 요청·자동화 설정·운영 상태·설정으로 분리하고, 업무별 Mail 타임라인과 변경 이력을 제공한다. 동일 Case의 수동 처리시간 측정 UI를 구현했으며 실제 Baseline은 측정 완료 전까지 달성값으로 기록하지 않는다.
