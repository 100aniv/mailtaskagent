...
* **선택적 Mail 연동:** 읽기 전용 테스트 Gmail Adapter Contract 구현, 실제 OAuth 연결과 제한 Label Query 검증
...
* 읽기 전용 테스트 Gmail Adapter는 공통 Mail Input Schema를 출력하도록 Core와 분리했으며,
  실제 OAuth 연결 후 제한 Label의 비식별 합성 Mail 2건을 회사 LLM Live로 처리해
  `CREATE_TASK`와 동일 Thread `UPDATE_TASK`, 실패 0건을 확인했다.
