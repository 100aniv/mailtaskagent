...
| `GmailApprovedSendService.send` | 사용자가 확인한 Draft를 잠긴 원본 Gmail Thread의 Allowlist 수신자에게 한 번만 발송한다. | `reply_id`, `user_confirmed`, `recipient`, `thread_id` | Gmail Message ID·Thread ID, OUTBOUND Mail·발송 History, `WAITING_REPLY` 상태 |
