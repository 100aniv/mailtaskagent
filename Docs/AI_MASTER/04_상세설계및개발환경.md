...
```text
당신은 메일 기반 개인 업무요청 관리 Agent이다.

당신의 임무는 새로운 메일 한 건을 단순히 분류하거나 요약하는 것이 아니다.

현재 메일, 선행·후행 메일, Mail Thread 정보, 현재 Task 상태,
Task 변경 이력 및 사용자가 확정한 정보를 함께 분석하여
다음에 수행해야 할 업무 Action을 결정해야 한다.
...
2. Mail ID, Conversation ID, 수신·발신 방향, 발신자, 수신자,
   제목 및 시각 등의 Metadata를 우선 활용한다.

3. 새로운 메일이 다음 중 어떤 유형인지 판단한다.
   - 신규 업무
   - 기존 업무의 후속 요청
   - 기존 업무의 요청사항 변경
   - 기존 업무의 기한 변경
...
5. 관련 Task 후보가 여러 개이거나 다음 조건에 해당하면 ASK_USER를 선택한다.
   - 기한 표현이 모호한 경우
   - 완료 또는 취소 여부가 명확하지 않은 경우
   - 신규 업무인지 기존 업무인지 확정하기 어려운 경우
   - 중요 Task 정보 변경의 근거가 부족한 경우

6. 완료·취소 처리, 모호한 기한 변경 및 기존 Task 연결 등
   중요한 변경은 근거가 명확한 경우에만 제안한다.

7. 날짜는 메일 수신 또는 발신 시각을 기준으로 해석한다.
   "다음 주 중", "가능한 시점" 등 모호한 날짜는 임의로 확정하지 않는다.

8. 사용자가 직접 수정하거나 승인한 Task 정보는
   가장 최신의 확정 정보로 취급한다.

9. 동일 mail_id가 이미 처리된 경우 중복 Task 생성이나
   중복 상태 변경을 수행하지 않는다.

10. 메일을 발송·삭제·이동하거나 DB를 직접 변경하지 않는다.
    실제 변경은 검증된 Application Logic이 수행한다.

11. 결과는 지정된 JSON Schema 형식으로 반환한다.

12. 최종 결과에는 다음 항목을 포함한다.
    - 선택한 Action
    - 대상 Task ID
    - 생성 또는 변경할 내용
    - 판단 근거
...
5. Metadata만으로 판단하기 어려운 경우 회사 제공 LLM API를 이용하여 의미적 연관성을 판단한다.
...
| `analyze_mail`              | 메일에서 업무 요청 여부와 주요 정보를 추출한다.                      | `mail_id`, `sender`, `subject`, `body`, `received_at`                                                 | 업무 여부, 요청사항, 요청자, 기한, 회신 필요 여부 |
...
| **구분**              | **선정 전략/기술**                                               | **선정 사유**                                                                                                                               |
| ------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **LLM Model**       | **회사 제공 LLM API 모델**                                       | 회사에서 제공하는 표준 LLM 환경을 사용하며 한국어 업무메일 이해, 요청사항·기한 추출, Mail–Task 관계 및 Action 판단 성능을 검증한다. 정확한 모델명은 API 연동 시 확정한다.                           |
...
| **Prompt Strategy** | **Role Prompt + Few-Shot + State/Action 제한 Prompt**        | 대표 Mail Case를 예시로 제공하고 선택 가능한 Action을 제한하여 자연어 표현의 다양성을 처리하고 Hallucination을 줄인다.                                                        |
| **Output Parsing**  | **Structured Output 또는 JSON Schema + Pydantic Validation** | Agent가 반환하는 Action, Task ID, 기한, 상태 및 변경값을 정형화하며 검증 실패 결과는 DB에 반영하지 않는다.                                                                |
...
| **Monitoring**      | **Python Logging + SQLite Audit History**                  | Agent 판단, API 결과, 함수 실행, Task 변경, 사용자 결정 및 오류를 추적한다.                                                                                    |
| **Model Training**  | **별도 Training 및 Fine-tuning 미수행**                          | 회사 제공 LLM API를 사용하고 System Prompt, Few-Shot Example, Context 및 Structured Output을 이용하여 Agent 동작을 제어한다. Dataset은 모델 학습이 아니라 성능 평가에 활용한다. |
...
* **선택적 Mail 연동:** 테스트 전용 Gmail

* **Monitoring:** Python Logging 및 SQLite Audit History
...
* `.gitignore`에 `.env`, SQLite 운영파일 및 로그파일 등을 포함한다.
...
* 구현 일정이 허용되는 경우 테스트 전용 Gmail을 추가 연동할 수 있다.
