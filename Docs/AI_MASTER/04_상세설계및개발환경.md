...
현재 시연 기준선에서는 위 역할을 한 번에 LLM에 맡기지 않고 두 층으로 분리한다. M-01의 회사
LLM Prompt는 Mail 의미와 Intent만 구조화하고, M-02 후보 검색과 M-03 Action 결정은 검증 가능한
Python Application Logic이 수행한다. 최종 MVP에서는 별도 Task Context Agent가 검색된 제한
후보의 관계와 Action 제안값만 구조화하며, Python M-03의 최종 결정과 DB 변경 권한은 유지한다.
...
7. 별도 Task Context Agent가 `SAME_TASK`, `NEW_TASK`, `AMBIGUOUS`, 후보 안의 Task ID,
   기존 7 Action 중 제안값, 신뢰도, 근거와 선택적 `rewritten_query`를 반환한다.

8. 첫 판단이 모호하거나 신뢰도 기준 미만이면 rewritten query로 정확히 최대 1회 재검색·재판단한다.
   재판단도 불확실하거나 API·Schema 오류가 나면 자동 연결하지 않고 `ASK_USER`로 전환한다.

9. Python M-03은 현재 Mail Context와 Task State, 검증된 LLM 제안값을 바탕으로 다음 Action 중 하나를 결정한다.
...
최종 MVP 구현 시 `retrieval_query`, `retrieved_task_contexts`, `task_context_decision`,
`retry_count`를 추가한다. 현재 코드에는 아직 없는 계획 State이며 신규·전체 회귀 통과 후 구현
완료로 전환한다.
...
| `retrieve_task_contexts`    | 동일 Thread로 확정할 수 없을 때 제한된 활성 Task Context top-k를 검색한다. *(최종 MVP 추가 예정)* | `query`, `requester`, `top_k` | 후보 Task, 최근 Mail·History, 검색 점수·근거 |
| `judge_task_context`        | 검색 후보와 현재 Mail의 관계 및 Action 제안값을 구조화한다. *(최종 MVP 추가 예정)* | `current_mail`, `mail_analysis`, `retrieved_task_contexts`, `retry_count` | 관계, 후보 안의 선택 Task ID, 제안 Action, 신뢰도, 근거, rewritten query |
...
보고 관계를 제안한다. 이 설계는 2026-09-01 현재 구현·테스트 전이다.
...
아래는 Vector DB·외부 Embedding·사내 문서 RAG를 사용하지 않는 이유와, 별도로 최종 MVP에
추가할 SQLite Task Context RAG의 안전 경계다.
...
* LLM의 Action은 제안값이며 Python M-03, 상태 전이와 Human-in-the-loop를 우회하지 못함
...
| **Task Context RAG** | **SQLite Structured Retrieval + 별도 Task Context Agent** | 동일 Thread로 확정할 수 없을 때 top-k Task·최근 Mail·History만 전달하고 최대 1회 Query Rewrite 후에도 불확실하면 `ASK_USER`로 전환한다. 현재 최종 MVP 추가 예정이며 Vector DB를 사용하지 않는다. |
...
* **선택적 Mail 연동:** 읽기 전용 테스트 Gmail Adapter Contract, 실제 OAuth 연결, 제한 Label 신규 유입과 Task 연결 Thread의 수신·발신 후속 Mail 추적 검증
...
* `TASK_CONTEXT_RAG_MAX_RETRIES`는 최종 MVP 구현 시 반드시 `1`로 제한하며, 현재 환경에는 아직
  활성 설정으로 적용되지 않았다.
...
* M-01 LLM은 자연어 해석과 Intent 구조화를 담당하고, 최종 MVP의 별도 Task Context Agent는
  검색 후보의 관계와 Action 제안값만 구조화한다. 최종 Action 판단은 Python Application Logic이 수행한다.
...
* SQLite Task Context RAG는 최종 MVP에 추가하되 Vector DB, 외부 Embedding, Multi-Agent는
