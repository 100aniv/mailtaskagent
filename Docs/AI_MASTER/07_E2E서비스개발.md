...
현재 자동 테스트는 제품형 Dashboard, 운영/시연 모드와 DB 격리, Gmail 수신·발신 Thread 추적, 운영 자동화·복구, SQLite WAL·동시 동기화 단일 실행 잠금, 업무별 Mail 타임라인·History UI, Task Context RAG·ReAct, Agent Action Proposal·Python Safety Guard, Agent Trace, Mail-to-Action Draft와 Gmail 사용자 승인 발송 회귀를 포함하여 2026-09-09 최신 회귀 기준 `171 passed`입니다(2026-09-08 기준선 170 passed). 회사 LLM Mail 분석은 15/15 실행 단위·28/28 Action 단계, Task Context Agent Live 합성 검증은 3/3, Reply Planning은 3/3, 사용자 입력 기반 Draft 생성은 1/1, 별도 테스트 Gmail 비식별 합성 Mail 수용시험은 20/20 통과했습니다. 사용자 승인 실제 Gmail 발송은 최초 검증 1건과 최종 재검증 1건에서 원본 Thread·수신자 잠금, Allowlist, 발송 History와 `WAITING_REPLY` 전환을 확인했습니다. 이 수치는 정의된 합성 검증 범위이며 실제 Mailbox 전체 성능으로 확대 해석하지 않습니다. LLM Intent 진단은 13/15였지만 Agent Proposal과 Python Guard를 거친 최종 Action, 필드와 Task 연결 결과는 기대값과 일치했습니다.
