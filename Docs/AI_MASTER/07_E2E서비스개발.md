...
현재 자동 테스트는 제품형 Dashboard, 운영/시연 모드와 DB 격리, Gmail 수신·발신 Thread 추적, 운영 자동화·복구, SQLite WAL·동시 동기화 단일 실행 잠금, 업무별 Mail 타임라인·History UI, Task Context RAG·ReAct, Agent Action Proposal·Python Safety Guard와 Agent Trace 회귀를 포함하여 `149 passed`입니다. 회사 LLM Mail 분석은 15/15 실행 단위·28/28 Action 단계, Task Context Agent Live 합성 검증은 3/3, 별도 테스트 Gmail 비식별 합성 Mail 수용시험은 20/20 통과했습니다. 이 수치는 정의된 합성 검증 범위이며 실제 Mailbox 전체 성능으로 확대 해석하지 않습니다. LLM Intent 진단은 13/15였지만 Agent Proposal과 Python Guard를 거친 최종 Action, 필드와 Task 연결 결과는 기대값과 일치했습니다.
...
* Post-MVP의 기능 확장은 회신 방식을 판단하고 사용자 입력으로 Draft를 만드는 Mail-to-Action
  Draft부터 시작합니다. 이후 테스트 Gmail에서 사용자 명시 승인 발송과 안전장치를 검증한 뒤
  Outlook·사내 운영 Adapter로 전환합니다. 현재 MVP에는 Draft 생성과 실제 발송을 포함하지 않습니다.
