...
현재 자동 테스트는 제품형 Dashboard, 운영/시연 모드와 DB 격리, Gmail 수신·발신 Thread 추적, 운영 자동화·복구, 업무별 Mail 타임라인과 History UI 회귀를 포함하여 `120 passed`이며 Live 평가 실행 오류는 0건입니다. 별도 테스트 Gmail의 비식별 합성 Mail 수용시험도 20/20 통과했습니다. KPI의 100% 수치는 현재 정의된 15개 합성 Mail과 평가 대상 필드·Task 연결에만 해당하며 실제 Mailbox 전체 성능으로 확대 해석하지 않습니다. LLM Intent 진단은 13/15였지만 Python M-03의 최종 Action, 필드와 Task 연결 결과는 기대값과 일치했습니다.
