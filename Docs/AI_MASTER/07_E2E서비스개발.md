...
* **현재 검증된 아키텍처 핵심:** 합성 Mail 입력부터 회사 LLM 의미 분석, Metadata 우선·SQLite Task Context RAG, 최대 1회 ReAct 재판단, Task Context Agent의 관계·대상·Action 선택, Python Application Logic의 Payload 구성·Safety Guard, Pydantic Validation, Human-in-the-loop, SQLite Task·History 저장, 실행 결과 관찰과 Streamlit Agent Trace까지 연결한 단일 Agent Core E2E
...
* **최종 MVP Agentic 아키텍처:** 동일 Thread로 확정할 수 없을 때 SQLite의 Task·최근 Mail·History
  ...
  저신뢰·모호한 첫 판단은 Query를 최대 1회 재작성·재검색하며, 이후에도 불확실하거나 오류가
...
Action에 한해 DB Transaction을 수행합니다.
...
현재 자동 테스트는 제품형 Dashboard, 운영/시연 모드와 DB 격리, Gmail 수신·발신 Thread 추적, 운영 자동화·복구, SQLite WAL·동시 동기화 단일 실행 잠금, 업무별 Mail 타임라인·History UI, Task Context RAG·ReAct, Agent Action Proposal·Python Safety Guard, Agent Trace와 Mail-to-Action Draft 회귀를 포함하여 `158 passed`입니다. 회사 LLM Mail 분석은 15/15 실행 단위·28/28 Action 단계, Task Context Agent Live 합성 검증은 3/3, Reply Planning은 3/3, 사용자 입력 기반 Draft 생성은 1/1, 별도 테스트 Gmail 비식별 합성 Mail 수용시험은 20/20 통과했습니다. 이 수치는 정의된 합성 검증 범위이며 실제 Mailbox 전체 성능으로 확대 해석하지 않습니다. LLM Intent 진단은 13/15였지만 Agent Proposal과 Python Guard를 거친 최종 Action, 필드와 Task 연결 결과는 기대값과 일치했습니다.
...
* Post-MVP의 첫 기능 확장인 Mail-to-Action Draft는 구현을 완료했습니다. Agent가 최신 수신
  Mail·Task·History로 회신 방식을 판단하고 날짜·값·승인 등 필요한 사용자 입력을 선택해 초안을
  저장합니다. 실제 발송은 하지 않습니다. 다음 단계는 테스트 Gmail에서 사용자 명시 승인 발송과
  수신자·Reply-All·발송 History 안전장치를 검증한 뒤 Outlook·사내 운영 Adapter로 전환하는 것입니다.
