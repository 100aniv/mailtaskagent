...
최종 회귀 검증은 2026-09-09 최신 UI 회귀를 포함한 `pytest 171 passed`다(2026-09-08 기준선 `170 passed`). 회사 LLM Mail 분석 Live 15/15 실행 단위·28/28 Action 단계, Task Context Agent Live 합성 검증 3/3, Agent Action Proposal·Python Safety Guard 신규 회귀 13건, 별도 테스트 Gmail 비식별 합성 Mail 20/20 수용시험, Windows Scheduler 반복 실행 성공(`LastTaskResult=0`)과 SQLite `quick_check=ok`를 확인했습니다. 최종 MVP의 Mail-to-Action은 Reply Planning Live 3/3, 사용자 입력 기반 Draft 생성 1/1과 Streamlit 판단→입력→초안→승인 흐름을 통과했습니다. 사용자 승인 실제 Gmail 발송은 최초 검증 1건과 최종 재검증 1건에서 원본 발신자·Thread 잠금, Send Allowlist, 중복 방지, OUTBOUND Mail·발송 History 저장과 `WAITING_REPLY` 전환을 확인했습니다. Read-only 동기화와 승인 발송의 OAuth Token은 서로 분리합니다.
