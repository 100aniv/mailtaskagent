...
CAPTIONS = [
    ...
    Caption(*_open[0], "SQLite Task Context RAG + 제한적 ReAct | Workflow는 실행 뼈대이고, 규칙으로 확정할 수 없는 관계·대상·Action은 Agent가 판단합니다."),
    Caption(*_rag[0], "Retrieve | 관련 Task·최근 Mail 3건·History 5건·사용자 결정을 SQLite에서 검색합니다. Vector DB가 아닌 Structured Retrieval입니다."),
    Caption(*_cmp[0], "Reason | Bounded Multi-Hypothesis Deliberation으로 후보 생성과 평가를 분리했습니다. Tree of Thoughts 아이디어를 참고했지만 Full ToT는 아닙니다."),
    Caption(*_cmp[1], "M-03 Evaluate | 지지도 0.70 대 0.60, 차이 0.10은 LLM이 아니라 Python이 계산합니다. 화면에 남는 것은 원시 Chain-of-Thought가 아니라 구조화된 판단 근거입니다."),
    Caption(*_cmp[2], "Self-Correction | Query Rewrite → Context 재검색 → 재판단, 최대 1회. 차이 0.10은 재판단 후에도 그대로였습니다."),
    Caption(*_hitl[0], "Act–Observe | Python Guard 검증 → DB 실행 또는 ASK_USER → 실제 저장 결과 재조회. 실제 저장된 이벤트입니다."),
...
def section_cards() -> list[tuple[str, str, str, str]]:
    ...
    return [
        ...
        (
            "02",
            "Agent 추론 로그",
            "SQLite Task Context RAG + 제한적 ReAct Self-Correction",
